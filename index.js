...
const particle_size = Math.floor(4096 / 100) //Size in pixels of each particle
...
<line x1="0" y1="${Y}" x2="${svgWidth}" y2="${Y}" stroke="#00F0FF"  stroke-opacity="1" shape-rendering="cripsEdges"/>
    ...
    await sharp(Buffer.from(coordinateSVG)).resize(4096).toFile('tmp_coords.png');
    await sharp('tmp_coords.png').composite([{ input: 'NFT.png', blend: 'overlay' }]).toFile('NFT_coords.png', (err, info) => { console.log(err,info) });
    await sharp('NFT.png').extract({
        /** zero-indexed offset from left edge */
        left: Math.floor((X*4090/100)-particle_size),
        /** zero-indexed offset from top edge */
        top: Math.floor((Y*4090/100)-particle_size),
